...
function checkLoginCode() {
	...
	$("#login").on("click", function() {
		...
		if (username == "" || pwd == "") {
			...
			var posturl = 'http://47.75.78.86:8089/user/login';
            // var posturl = process.env.CHANNEL_BACKEND_HOST + ":" + process.env.CHANNEL_BACKEND_PORT;
...
function checkUser(username) {
	return function(data) {
		...
		if (StringEqual(data.success, true)) {
			...
			user.push(apiChannle);
			console.log(apiChannle);
			setCookie("apiChannle", JSON.stringify(user));
